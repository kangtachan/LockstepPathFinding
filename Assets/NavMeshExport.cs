...
//
        ...
        //AssetDatabase.Refresh();
        var go = new GameObject("ExportMesh");
        go.AddComponent<MeshRenderer>();
        go.AddComponent<MeshFilter>().mesh = mesh;
        Debug.LogWarning("NavMesh triCount:" + mesh.triangles.Length / 3 + "VerticeCount :" + mesh.vertices.Length);
    }
    ...
    private static void Test(){
        ...
        for (int i = 0; i < triangles.Length; i += 3) {
            ...
            if (IsInside(worldVectors[triangles[i]], worldVectors[triangles[i + 1]], worldVectors[triangles[i + 2]],
                checkPoint)) {
                _isInside = true;
                break;
            }
    ...
    private static void ExportNavData(GameObject obj){
        ...
        sb.Append("local nav = {\n");
        for (int i = 0; i < triangles.Length; i += 3) {
            sb.AppendFormat("\t{{{0},{1},{2}}},\n", _VectorToLua(worldVectors[triangles[i]]),
                _VectorToLua(worldVectors[triangles[i + 1]]), _VectorToLua(worldVectors[triangles[i + 2]]));
        }
